            ...
            <div className="space-y-4 sm:space-y-6">
              {blogPosts.map((post, index) => (
                <div
                  ...
                  className="bg-white rounded-lg border border-slate-200 shadow-sm hover:shadow-lg transition-all duration-300 hover:-translate-y-1 cursor-pointer p-4 sm:p-6 animate-slide-up"
                  style={{ animationDelay: `${index * 100}ms` }}
                  tabIndex={0}
                  role="button"
                  aria-label={`Read blog post: ${post.title}`}
                >
                  <h2 className="text-lg sm:text-xl md:text-2xl font-semibold text-slate-900 hover:text-brand-teal transition-colors mb-2 font-display">
                    {post.title}
                  </h2>
                  <p className="text-sm sm:text-base text-slate-600 mb-3 sm:mb-4 leading-relaxed">
                    {post.excerpt}
                  </p>
                  <div className="flex flex-wrap gap-3 sm:gap-4 text-xs sm:text-sm text-slate-500 mb-3 sm:mb-4">
